...
const packet = () => ({
  ...
  droppedSamples: 0,
  manualAlarm: false,
  readings: [{ channel: "mq2", value: 700, state: "PENDING" }],
});
test("contrato canônico, sem idade nem campos extras no hash de idempotência", () => {
  const p = packet();
  assert.deepEqual(
    telemetry({ ...p, extra: "ignored", ageMs: 10 }),
    telemetry(p),
  );
});
test("rejeita tipos, NaN, ADC fora da faixa, canal duplicado e unidade disfarçada", () => {
  for (const value of [-1, 4096, "700", NaN, null])
    assert.throws(() =>
      telemetry({
        ...packet(),
        readings: [{ channel: "mq2", value, state: "ALARM" }],
      }),
    );
  ...
  assert.throws(() =>
    telemetry({
      ...packet(),
      readings: [...packet().readings, ...packet().readings],
    }),
  );
  assert.throws(() =>
    telemetry({
      ...packet(),
      readings: [{ channel: "mq2", value: 700, state: "vermelho" }],
    }),
  );
...
test("limites: histerese obrigatória e confirmação limitada", () => {
  assert.deepEqual(sensorConfig({ high: 700, low: 580, confirmMs: 5000 }), {
    high: 700,
    low: 580,
    confirmMs: 5000,
  });
  for (const c of [
    { high: 700, low: 700, confirmMs: 5000 },
    { high: 4096, low: 1, confirmMs: 100 },
    { high: 700, low: 0, confirmMs: 0 },
  ])
    assert.throws(() => sensorConfig(c));
});
test("dashboard não transforma offline, falta de dados ou config pendente em normal", () => {
  ...
  assert.equal(dashboardStatus(row, now), "NORMAL");
  assert.equal(dashboardStatus({}, now), "SEM_DADOS");
  assert.equal(dashboardStatus(row, now + 90001), "OFFLINE");
  assert.equal(
    dashboardStatus({ ...row, desired_version: 2 }, now),
    "CONFIG_PENDENTE",
  );
  assert.equal(
    dashboardStatus({ ...row, manual_alarm: 1, desired_version: 2 }, now),
    "ALARME",
  );
...
test("credenciais têm limite em bytes e chaves aleatórias são comparadas por hash", () => {
  ...
  assert.throws(() =>
    credentials({ email: "x@y.z", password: "á".repeat(40) }),
  );
  const a = token(),
    b = token();
  assert.notEqual(a, b);
  assert(equalHash(hash(a), hash(a)));
  assert(!equalHash(hash(a), hash(b)));
  assert(!equalHash(undefined, undefined));
});
test("config falha fechada sem DB e HTTPS em produção", () => {
  ...
  assert.equal(
    loadConfig({
      ...env,
      NODE_ENV: "production",
      APP_ORIGIN: "https://mqfire.example",
    }).allowRegistration,
    false,
  );
});
