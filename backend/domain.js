...
export function credentials(body) {
  ...
  if (
    ...
    throw new HttpError(400, "Senha: mínimo 12 caracteres e máximo 72 bytes.");
...
export function telemetry(body) {
  ...
  if (typeof body.bootId !== "string" || !/^[a-f0-9]{16}$/.test(body.bootId))
    throw new HttpError(400, "bootId inválido.");
  ...
  if (typeof body.manualAlarm !== "boolean")
    throw new HttpError(400, "manualAlarm inválido.");
  if (
    !Array.isArray(body.readings) ||
    body.readings.length < 1 ||
    body.readings.length > 8
  )
    throw new HttpError(400, "Envie de 1 a 8 canais.");
  const channels = new Set();
  for (const r of body.readings) {
    requireObject(r);
    identifier(r.channel, "Canal");
    integer(r.value, 0, 4095, "ADC");
    if (!["NORMAL", "PENDING", "ALARM", "WARMUP", "FAULT"].includes(r.state))
